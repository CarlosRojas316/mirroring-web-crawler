...
import (
	...
	"github.com/CarlosRojas316/mirroring-web-crawler/errors"
	"github.com/CarlosRojas316/mirroring-web-crawler/storage"
)
...
func (c *Crawler) crawlWorker(ctx context.Context, nextURLs []string) {
	...
	signal.Notify(ctrl_c, os.Interrupt, syscall.SIGINT)
	...
	for _, u := range nextURLs {
		...
		select {
		case <-ctrl_c:
			fmt.Println("Exiting webcrawler by Ctrl-C")
			os.Exit(1)
...
func (c *Crawler) crawlURL(urlToCrawl, filePath string) ([]byte, error) {
	// zap.L().Warn("Crawling URL %s", zap.String("URL", urlToCrawl))
